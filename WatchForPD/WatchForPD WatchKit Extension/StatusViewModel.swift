...
class StatusViewModel: NSObject {
    ...
    func getLastStatus() {
        ...
        if let canGetStatus = context?["canGetStatus"] as? Bool {
            if canGetStatus {
    ...
    func listenUpdateStatus() {
        dataManager.listenUpdateStatus { [weak self] (receivedDict) -> Void in
            if let strongSelf = self {
                if let canGetStatus = receivedDict["canGetStatus"] as? Bool {
                    if canGetStatus {
    ...
    func pullStatus() {
        dataManager.pullStatus({ [weak self] (replyDict) -> Void in
            if let strongSelf = self {
                if let canGetStatus = replyDict["canGetStatus"] as? Bool {
                    if canGetStatus {
    ...
    func pullRecordContextsWithNumber(number: Int){
        dataManager.pullRecordsWithNumber(number, replyHandler: { [weak self] (reply) -> Void in
            if let strongSelf = self {
                if let canGetRecord = reply["canGetRecord"] as? Bool {
                    if canGetRecord {
